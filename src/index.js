...
class UI {
  ...
  renderTask(selected) {
    ...
    selected.task.forEach((task) => {
      ...
      if (task.completed == false) {
        li.innerHTML = `<div class="task-detail">
                        <div class="task">
  ...
  toggleCompleted(selected, e) {
    if (e.target.classList.contains("task-status")) {
      ...
      selectedTask.forEach((item) => {
        if (
          item.completed === true &&
          item.todo === e.target.nextElementSibling.textContent
        ) {
          ...
          e.target.setAttribute(
            "class",
            "task-status fa-regular fa-circle-check"
          );
  ...
  showTaskDropDown(selected, e) {
    if (e.target.classList.contains("fa-caret-left")) {
      let project = myProjects.find(
        (item) => item.title === heading.textContent
      );
